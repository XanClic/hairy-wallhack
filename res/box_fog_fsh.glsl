...
uniform float enlightenment, alpha;
...
void main(void)
{
  ...
  out_mi = vec4(mix(col, vec3(0.7, 0.7, 0.7), smoothstep(0.99, 0.999, gl_FragCoord.z))      , 1.0);
  out_hi = vec4(mix(col, vec3(0.0, 0.0, 0.0), smoothstep(0.99, 0.999, gl_FragCoord.z)) / 5.0, 1.0);
}
